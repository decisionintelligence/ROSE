...
class Learner(GetAttr):
    ...
    def __init__(self, dls, model, 
        ...
        self.lp_model_forward= self.lp(self.model_forward)
    ...
    def train_step(self, batch):
        ...
        if self.n_embedding!=0:
            ...
            # compute loss
            loss_reconstruct = self.loss_func(pred, self.yb)
    ...
    def model_forward(self):
        self('before_forward')
        if self.n_embedding!=0:
            pred_96, pred_192, pred_336, pred_720, self.xe, self.xq= self.model(self.xb)
            if self.target_points ==96:
                self.pred = pred_96
            elif self.target_points ==192:
                self.pred = pred_192
            elif self.target_points ==336:
                self.pred = pred_336
            elif self.target_points ==720:
                self.pred = pred_720
            self('after_forward')
            return self.pred, self.xe, self.xq
        else:
    ...
    def test_step(self, batch):
        ...
        if self.n_embedding!=0:
            # pred, xe, xq = self.model_forward()
            pred, xe, xq = self.lp_model_forward()
        else:
            pred = self.model_forward()
        return pred, self.yb
    ...
    def fine_tune(self, n_epochs, base_lr=None, freeze_epochs=1, pct_start=0.3,freeze_embedding=0):
        ...
        if n_epochs > 0:
            ...
            if self.n_embedding and freeze_embedding:
                for param in get_model(self.model).vq_embedding.parameters(): param.requires_grad = False
    ...
    def freeze(self):
        ...
        """
        if hasattr(get_model(self.model), 'head_720'): 
            ...
            # get_model(self.model).task_token_prompt.requires_grad = True
            get_model(self.model).u.requires_grad = True
            get_model(self.model).v.requires_grad = True
        if hasattr(get_model(self.model), 'head_336'): 
            # print('model head is available')
            for param in get_model(self.model).parameters(): param.requires_grad = False
            ...
            get_model(self.model).u.requires_grad = True
            get_model(self.model).v.requires_grad = True
        if hasattr(get_model(self.model), 'head_192'): 
            # print('model head is available')
            for param in get_model(self.model).parameters(): param.requires_grad = False
            ...
            get_model(self.model).u.requires_grad = True
            get_model(self.model).v.requires_grad = True
        if hasattr(get_model(self.model), 'head_96'): 
            # print('model head is available')
            for param in get_model(self.model).parameters(): param.requires_grad = False
    ...
    def unfreeze(self):
        ...
        for param in get_model(self.model).vq_embedding.parameters(): param.requires_grad = False
        # get_model(self.model).dec_pos_embedding.requires_grad = True
        ...
        # for param in get_model(self.model).backbone.encoder.parameters(): param.requires_grad = True
        # get_model(self.model).task_token_predict.requires_grad = True
    ...
    def load(self, fname, with_opt=False, device='cuda', strict=True, **kwargs):
        ...
        # self.model=transfer_weights(fname, self.model, device=device)
    ...
    def visual_model(self, dl, save_path=None):
        preds = []
        input = []
        truth = []
        mse = []
        for num , batch in enumerate(dl):
            self.xb, self.y = batch
            device = self.xb.device
            input.append(self.xb.detach().numpy())
            truth.append(self.y.detach().numpy())
            self.xb = self.xb.to('cuda:0')
            self.y = self.y.to('cuda:0')

            # pred, xe, xq = self.model_forward()
            pred = self.model_forward()
            preds.append(pred.detach().cpu().numpy())
            
            mse.append(np.mean((truth[-1] - preds[-1])**2))
        min_index = np.argmin(mse)
        min_index = 8544
        num_c = input[0].shape[2]
        for i in range(num_c):
            gt = np.concatenate((input[min_index][0, :, i], truth[min_index][0, :, i]), axis=0)
            pred = np.concatenate((input[min_index][0, :, i], preds[min_index][0, :, i]), axis=0) 
            visual(gt[416:], pred[416:], channel=i, path=save_path, index=min_index)
...
def visual(true, preds, channel , path='./pic/test.pdf', index=0):
    '''
    ...
    if preds is not None:
        plt.plot(preds, label="Prediction", linewidth=1.5, c='darkorange')
    plt.plot(true, label="GroundTruth", linewidth=1.5, c='royalblue')
    # plt.vlines(96, np.min(preds)-0.1, np.max(preds)+0.1, linestyles='dashed', colors='gray')
    plt.legend()
    path = path + "_b" +str(index)+ "_"+ str(channel) + '.pdf'
