...
__all__ = ['OneCycleLR', 'LRFinderCB', 'LinearLR', 'ExponentialLR','StepLR','ExponentialLR','MultiStepLR','LinearLR','CosineAnnealingLR']
...
class OneCycleLR(Callback):
    ...
    def before_fit(self):
        ...
        self.scheduler = lr_scheduler.StepLR(optimizer = self.opt, step_size=10, gamma=0.1)
...
class StepLR(Callback):
    ...
    def before_fit(self):
        if not self.steps_per_epoch: self.steps_per_epoch = len(self.dls.train)
        self.lrs = []  # store lr values
        self.scheduler = lr_scheduler.StepLR(optimizer = self.opt, step_size=self.step_size, gamma=self.gamma)
...
class CosineAnnealingLR(Callback):
    ...
    def before_fit(self):
        if not self.steps_per_epoch: self.steps_per_epoch = len(self.dls.train)
        self.lrs = []  # store lr values
        self.scheduler = lr_scheduler.CosineAnnealingLR(optimizer = self.opt, T_max =  self.n_epochs * self.steps_per_epoch ,eta_min=2e-6, last_epoch=- 1, verbose=False)
...
class ExponentialLR(Callback):
    ...
    def before_fit(self):
        if not self.steps_per_epoch: self.steps_per_epoch = len(self.dls.train)
        self.lrs = []  # store lr values
        self.scheduler = lr_scheduler.ExponentialLR(optimizer = self.opt, gamma=self.gamma)
...
class MultiStepLR(Callback):
    ...
    def before_fit(self):
        if not self.steps_per_epoch: self.steps_per_epoch = len(self.dls.train)
        self.lrs = []  # store lr values
        self.scheduler = lr_scheduler.MultiStepLR(optimizer = self.opt, milestones=self.milestones, gamma=self.gamma)
...
class  LinearLR(Callback):
    ...
    def before_fit(self):
        if not self.steps_per_epoch: self.steps_per_epoch = len(self.dls.train)
        self.lrs = []  # store lr values
        self.scheduler = lr_scheduler.LinearLR(optimizer=self.opt,start_factor=self.start_factor,end_factor=self.end_factor,
