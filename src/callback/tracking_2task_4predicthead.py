...
class TrackTrainingCB(Callback):
    ...
    def initialize_recorder(self):
        recorder = {'epoch': [],  'train_loss': [],  'train_mse': [], 'train_pred_96': [], 'train_pred_192': [],'train_pred_336': [],'train_pred_720': []} 
        if self.valid_loss: 
            recorder['valid_loss'] = []
    ...
    def initialize_batch_recorder(self, with_metrics):
        batch_recorder = {'n_samples': [], 'batch_losses': [], 'batch_mses': [], 'batch_pred_losses_96': [],'batch_pred_losses_192': [],\
                          'batch_pred_losses_336': [], 'batch_pred_losses_720': [],'with_metrics': with_metrics}                                                         
    ...
    def after_epoch_train(self):         
        ...
        self.recorder['train_pred_96'].append(values['pred_loss_96'])
        self.recorder['train_pred_192'].append(values['pred_loss_192'])
        self.recorder['train_pred_336'].append(values['pred_loss_336'])
        self.recorder['train_pred_720'].append(values['pred_loss_720'])
    ...
    def after_epoch_valid(self):             
        ...
        self.recorder['valid_pred_loss'].append(values['pred_loss'])
        # save metrics after one epoch         
    ...
    def accumulate(self ):
        ...
        mse = self.mse*bs if self.mean_reduction_ else self.mse   
        pred_loss_96 = self.pred_loss_96*bs if self.mean_reduction_ else self.pred_loss_96 
        pred_loss_192 = self.pred_loss_192*bs if self.mean_reduction_ else self.pred_loss_192
        pred_loss_336 = self.pred_loss_336*bs if self.mean_reduction_ else self.pred_loss_336
        pred_loss_720 = self.pred_loss_720*bs if self.mean_reduction_ else self.pred_loss_720
        self.batch_recorder['batch_losses'].append(loss)
        self.batch_recorder['batch_mses'].append(mse)
        self.batch_recorder['batch_pred_losses_96'].append(pred_loss_96)
        self.batch_recorder['batch_pred_losses_192'].append(pred_loss_192)
        self.batch_recorder['batch_pred_losses_336'].append(pred_loss_336)
        self.batch_recorder['batch_pred_losses_720'].append(pred_loss_720)

        # print iter loss
        if self.iter % 500 == 0:
            n = sum(self.batch_recorder['n_samples'])  
            cur_loss = sum(self.batch_recorder['batch_losses']).item()/n
            cur_mse = sum(self.batch_recorder['batch_mses'])/n  # averaging
            cur_pred_loss_96 = sum(self.batch_recorder['batch_pred_losses_96'])/n  # averaging
            cur_pred_loss_192 = sum(self.batch_recorder['batch_pred_losses_192'])/n  # averaging
            cur_pred_loss_336 = sum(self.batch_recorder['batch_pred_losses_336'])/n  # averaging
            cur_pred_loss_720 = sum(self.batch_recorder['batch_pred_losses_720'])/n  # averaging
            print(f"{self.iter} iter loss: {cur_loss:.4f}  iter mse: {cur_mse:.4f} iter pred_loss_96: {cur_pred_loss_96:.4f} iter pred_loss_192: {cur_pred_loss_192:.4f} iter pred_loss_336: {cur_pred_loss_336:.4f} iter pred_loss_720: {cur_pred_loss_720:.4f}")
    ...
    def compute_scores(self):
        ...
        values['mse'] = sum(self.batch_recorder['batch_mses'])/n  # averaging
        values['pred_loss_96'] = sum(self.batch_recorder['batch_pred_losses_96'])/n  # averaging
        values['pred_loss_192'] = sum(self.batch_recorder['batch_pred_losses_192'])/n  # averaging
        values['pred_loss_336'] = sum(self.batch_recorder['batch_pred_losses_336'])/n  # averaging
        values['pred_loss_720'] = sum(self.batch_recorder['batch_pred_losses_720'])/n  # averaging
        ...
        # for func in self.metrics:             
            ...
            # values[func.__name__] = func(self.targs, self.preds).detach().numpy()   
