...
class TrackTrainingCB(Callback):
    ...
    def initialize_recorder(self):
        recorder = {'epoch': [],  'train_loss': [],  'train_mse': [], 'train_pred_loss': []} 
        if self.valid_loss: 
            recorder['valid_loss'] = []
    ...
    def initialize_batch_recorder(self, with_metrics):
        batch_recorder = {'n_samples': [], 'batch_losses': [], 'batch_mses': [], 'batch_pred_losses': [], 'with_metrics': with_metrics}                                                         
    ...
    def after_epoch_train(self):         
        ...
        self.recorder['train_pred_loss'].append(values['pred_loss'])
    ...
    def accumulate(self ):
        ...
        pred_loss = self.pred_loss*bs if self.mean_reduction_ else self.pred_loss        
        self.batch_recorder['batch_losses'].append(loss)
        self.batch_recorder['batch_mses'].append(mse)
        self.batch_recorder['batch_pred_losses'].append(pred_loss)
        ...
        if self.iter % 500 == 0:
            ...
            cur_pred_loss = sum(self.batch_recorder['batch_pred_losses'])/n  # averaging
            print(f"{self.iter} iter loss: {cur_loss:.4f}  iter mse: {cur_mse:.4f} iter pred_loss: {cur_pred_loss:.4f}")
    ...
    def compute_scores(self):
        ...
        values['pred_loss'] = sum(self.batch_recorder['batch_pred_losses'])/n  # averaging
