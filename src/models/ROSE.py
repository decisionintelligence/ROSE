...
class ROSE(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int,n_embedding:int, num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
                 n_layers:int=3, d_model=128, n_heads=16, shared_embedding=True, d_ff:int=256, num_slots:int=10, num_token=3,
    ...
    def initialize_weights(self):
        ...
        w1 = self.patch_embed.weight.data
        torch.nn.init.xavier_uniform_(w1.view([w1.shape[0], -1]))

        w2 = self.token_embed.weight.data
    ...
    def encoder_copy(self):
        for params_r, params_p in zip(self.encoder_reconstruct.parameters(), self.encoder_predict.parameters()):
            params_p.data.copy_(params_r.data)  # initialize
            params_p.requires_grad = False  # not update by gradient
    ...
    def forward(self, z):                             
        ...
        if self.mask_mode == "multi" or self.mask_mode == "freq_multi":
            ...
            bs, num_patch, n_vars, patch_len = z_masked.shape 

        #token
        z_token_predict=self.token_embed(z_original.permute(0,2,1,3).reshape(bs, n_vars, num_patch*patch_len))
        z_token_predict=z_token_predict.unsqueeze(1)          #[bs x 1 x nvars x d_model]

        if self.n_embedding!=0:
            embedding = self.vq_embedding.weight.data
            B, N, C, D  = z_token_predict.shape
            K, _ = embedding.shape
            embedding_broadcast = embedding.reshape(1, K, 1, 1, D)
            z_token_predict_broadcast = z_token_predict.reshape(B, 1, N, C, D)
            distance_predict = torch.sum((embedding_broadcast - z_token_predict_broadcast) ** 2, 4)
            # make C to the second dim
            z_token_predict_q = self.vq_embedding(torch.argmin(distance_predict, 1))
            # stop gradient
            z_token_predict_encoderinput = z_token_predict + (z_token_predict_q - z_token_predict).detach()
        else:
            z_token_predict_encoderinput = z_token_predict

        z_token_reconstruct_encoderinput = z_token_predict_encoderinput.unsqueeze(0).expand(mask_nums,-1,-1,-1,-1).reshape(-1, 1, n_vars, self.d_model)
        z_token_reconstruct_encoderinput = z_token_reconstruct_encoderinput.reshape(-1,n_vars,self.num_token,self.d_model).permute(0,2,1,3)
        z_token_predict_encoderinput = z_token_predict_encoderinput.reshape(-1,n_vars,self.num_token,self.d_model).permute(0,2,1,3)

        # patch embedding
        z_masked = self.patch_embed(z_masked)
        z_original = self.patch_embed(z_original)

        # concat prompt token
        z_masked = torch.cat((z_token_reconstruct_encoderinput, z_masked), dim=1)
        z_original = torch.cat((z_token_predict_encoderinput, z_original), dim=1)
        
        # shared backbone
        z_masked = self.backbone(z_masked)
        z_original = self.backbone(z_original)

        # aggregation
        if self.mask_mode == "multi" or self.mask_mode == "freq_multi":
            z_masked = z_masked.view((self.mask_nums ),-1, n_vars, self.d_model, num_patch + self.num_token).mean(0)     # z: [maks_nums x bs x nvars x d_model x num_patch]    

        # tower encoder
        self.encoder_copy()
        output_reconstruct = self.encoder_reconstruct(z_masked[:,:,:,self.num_token:])
        output_predict = self.encoder_predict(z_original[:,:,:,self.num_token:])
        output_predict = (output_predict - z_original[:,:,:,self.num_token:]).detach() + z_original[:,:,:,self.num_token:]


        # head
        output_reconstruct = self.head_reconstruct(output_reconstruct[:,:,:,:])
        output_predict_96 = self.head_96(output_predict[:,:,:,:])
        output_predict_192 = self.head_192(output_predict[:,:,:,:])
        output_predict_336 = self.head_336(output_predict[:,:,:,:])
        output_predict_720 = self.head_720(output_predict[:,:,:,:])

        if self.n_embedding!=0:
            return output_reconstruct,output_predict_96,output_predict_192,output_predict_336,output_predict_720, z_token_predict_q, z_token_predict
        else:
            return output_reconstruct,output_predict_96,output_predict_192,output_predict_336,output_predict_720
