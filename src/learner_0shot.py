...
class Learner(GetAttr):
    ...
    def model_forward(self):
        self('before_forward')
        if self.n_embedding!=0:
            pred_96, pred_192, pred_336, pred_720, res, self.xe, self.xq= self.model(self.xb)
            ...
            self.pred = self.model(self.xb, self.target_points)
            self('after_forward')
            return self.pred
    ...
    def load(self, fname, with_opt=False, device='cuda', strict=True, **kwargs):
        ...
        self.model=transfer_weights(fname, self.model, device=device, exclude_head=False)
